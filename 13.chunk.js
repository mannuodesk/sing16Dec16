...
var Generic = (function () {
    function Generic(_botTrainingService) {
        ...
        _botTrainingService.getAllGroups()
            ...
            console.log(jQuery(".divseparator").height());
            console.log(_this.blockGroupsModel);
        });
    }
    ...
    Generic.prototype.populateGalleryImage = function (event) {
        ...
        /*if ( this.isValidExtension(file) == false ) {
            console.log('Invalid file format!', 'Invalid file Format. Only ' + ImageComponent.ALLOWED_EXTENSIONS.join(', ') + ' are allowed.', 'error');
            return false;
        }
        if (this.isValidFileSize(file) == false) {
            console.log('Invalid file size!', 'Invalid file size. Max allowed is : ' + ImageComponent.ALLOWED_SIZE + ', your file is : ' + this.getFileSize(file) + ' Mb' , 'error');
            return;
        }*/
        console.log(file);
    ...
    Generic.prototype.populateSingleImage = function (input) {
        jQuery('#singleImageDiv').show();
        jQuery('#singleImageBtn').hide();
        if (input.files && input.files[0]) {
            var reader = new FileReader();
            reader.onload = function (e) {
                //jQuery('#singleImageDiv')
